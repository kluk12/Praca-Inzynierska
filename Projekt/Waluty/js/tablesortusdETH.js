...
function orderbook() {
    ajaxGet('https://bittrex.com/api/v1.1/public/getorderbook?market=usdt-eth&type=both', //
        function (response) {
...
function d() {
    ajaxGet('https://bittrex.com/api/v1.1/public/getmarketsummary?market=usdt-eth', //
        function (response) {
            response = JSON.parse(response);
            if (!response)
                return;
...
function inteval() {
    ...
    /* if (bittrex == null & ob == null)
     ...
     setTimeout(function () {
         if (ob == null)
             for (i in bittrex) {
                 if (ob.result[i].MarketName == "eth-ARK")
...
function ha() {
    ajaxGet('https://bittrex.com/api/v1.1/public/getmarkethistory?market=usdt-eth',
        function (response) {
            response = JSON.parse(response);
            if (!response)
                return;
